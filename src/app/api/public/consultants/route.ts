...
export async function GET() {
  ...
  const targetUrl = `${backendBaseUrl}/api/v1/public/consultants`;
  ...
  try {
    ...
    console.error("[consultants-proxy] backend unavailable", {
      targetUrl,
      error: error instanceof Error ? error.message : String(error)
    });
    return NextResponse.json({ success: false, message: "顾问展示服务暂时不可用" }, { status: 502 });
  }
}
